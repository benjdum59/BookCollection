...
public final class BookJSONBase: NSCoding {
  ...
  public var items: [BookJSONItems]?
  ...
  public required init(json: JSON) {
    ...
    if let itemsTmp = json[SerializationKeys.items].array { items = itemsTmp.map { BookJSONItems(json: $0) } }
  }
  ...
  required public init(coder aDecoder: NSCoder) {
    ...
    self.items = aDecoder.decodeObject(forKey: SerializationKeys.items) as? [BookJSONItems]
  }
